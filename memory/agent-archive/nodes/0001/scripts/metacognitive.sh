#!/bin/bash
# metacognitive.sh — HyperAgents 메타인지 엔진 v3
#
# 변경점 (v3):
...
if proposals:
    ...
    with open(meta_log, 'a') as f:
        f.write(f"\n## [{today}] 메타인지 v3\n")
        f.write(f"- **지표**: 성공률 {cron.get('successRate',0):.0f}% · 전달률 {cron.get('deliveryRate',0):.0f}% · 임계 {cron.get('critical',0)}개\n")
